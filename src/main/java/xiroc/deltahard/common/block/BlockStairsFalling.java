...
public class BlockStairsFalling extends BlockFalling
{
    public static final PropertyDirection FACING = BlockHorizontal.FACING;
    public static final PropertyEnum<BlockStairsFalling.EnumHalf> HALF = PropertyEnum.<BlockStairsFalling.EnumHalf>create("half", BlockStairsFalling.EnumHalf.class);
    public static final PropertyEnum<BlockStairsFalling.EnumShape> SHAPE = PropertyEnum.<BlockStairsFalling.EnumShape>create("shape", BlockStairsFalling.EnumShape.class);
    /**
     * B: .. T: xx
     * B: .. T: xx
     */
    ...
    public BlockStairsFalling(String name, IBlockState modelState, Material material, float hardness, float originalResistance)
    {
        super(material);
        this.setRegistryName(name);
        this.setDefaultState(this.blockState.getBaseState().withProperty(FACING, EnumFacing.NORTH).withProperty(HALF, BlockStairsFalling.EnumHalf.BOTTOM).withProperty(SHAPE, BlockStairsFalling.EnumShape.STRAIGHT));
        this.modelBlock = modelState.getBlock();
        this.modelState = modelState;
        this.setHardness(hardness);
        this.setResistance(originalResistance / 3.0F);
    ...
    private static List<AxisAlignedBB> getCollisionBoxList(IBlockState bstate)
    {
        ...
        boolean flag = bstate.getValue(HALF) == BlockStairsFalling.EnumHalf.TOP;
        list.add(flag ? AABB_SLAB_TOP : AABB_SLAB_BOTTOM);
        BlockStairsFalling.EnumShape blockstairs$enumshape = (BlockStairsFalling.EnumShape)bstate.getValue(SHAPE);

        if (blockstairs$enumshape == BlockStairsFalling.EnumShape.STRAIGHT || blockstairs$enumshape == BlockStairsFalling.EnumShape.INNER_LEFT || blockstairs$enumshape == BlockStairsFalling.EnumShape.INNER_RIGHT)
        {
            list.add(getCollQuarterBlock(bstate));
        }

        if (blockstairs$enumshape != BlockStairsFalling.EnumShape.STRAIGHT)
        {
    ...
    private static AxisAlignedBB getCollEighthBlock(IBlockState bstate)
    {
        ...
        switch ((BlockStairsFalling.EnumShape)bstate.getValue(SHAPE))
        {
            case OUTER_LEFT:
            default:
    ...
    public BlockFaceShape getBlockFaceShape(IBlockAccess worldIn, IBlockState state, BlockPos pos, EnumFacing face)
    {
        ...
        {
            return face == EnumFacing.UP == (state.getValue(HALF) == BlockStairsFalling.EnumHalf.TOP) ? BlockFaceShape.SOLID : BlockFaceShape.UNDEFINED;
        }
        else
        {
            BlockStairsFalling.EnumShape blockstairs$enumshape = (BlockStairsFalling.EnumShape)state.getValue(SHAPE);

            if (blockstairs$enumshape != BlockStairsFalling.EnumShape.OUTER_LEFT && blockstairs$enumshape != BlockStairsFalling.EnumShape.OUTER_RIGHT)
            {
    ...
    {
        return state.getValue(HALF) == BlockStairsFalling.EnumHalf.TOP;
    }
    ...
    public IBlockState getStateForPlacement(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer)
    {
        ...
        iblockstate = iblockstate.withProperty(FACING, placer.getHorizontalFacing()).withProperty(SHAPE, BlockStairsFalling.EnumShape.STRAIGHT);
        return facing != EnumFacing.DOWN && (facing == EnumFacing.UP || (double)hitY <= 0.5D) ? iblockstate.withProperty(HALF, BlockStairsFalling.EnumHalf.BOTTOM) : iblockstate.withProperty(HALF, BlockStairsFalling.EnumHalf.TOP);
    }
    ...
    {
        IBlockState iblockstate = this.getDefaultState().withProperty(HALF, (meta & 4) > 0 ? BlockStairsFalling.EnumHalf.TOP : BlockStairsFalling.EnumHalf.BOTTOM);
    ...
    public int getMetaFromState(IBlockState state)
    {
        ...
        if (state.getValue(HALF) == BlockStairsFalling.EnumHalf.TOP)
        {
            i |= 4;
        }
    ...
    private static BlockStairsFalling.EnumShape getStairsShape(IBlockState p_185706_0_, IBlockAccess p_185706_1_, BlockPos p_185706_2_)
    {
        ...
        if (isBlockStairs(iblockstate) && p_185706_0_.getValue(HALF) == iblockstate.getValue(HALF))
        {
            ...
            if (enumfacing1.getAxis() != ((EnumFacing)p_185706_0_.getValue(FACING)).getAxis() && isDifferentStairs(p_185706_0_, p_185706_1_, p_185706_2_, enumfacing1.getOpposite()))
            {
                ...
                {
                    return BlockStairsFalling.EnumShape.OUTER_LEFT;
                }

                return BlockStairsFalling.EnumShape.OUTER_RIGHT;
            }
        }
        ...
        if (isBlockStairs(iblockstate1) && p_185706_0_.getValue(HALF) == iblockstate1.getValue(HALF))
        {
            ...
            if (enumfacing2.getAxis() != ((EnumFacing)p_185706_0_.getValue(FACING)).getAxis() && isDifferentStairs(p_185706_0_, p_185706_1_, p_185706_2_, enumfacing2))
            {
                ...
                {
                    return BlockStairsFalling.EnumShape.INNER_LEFT;
                }

                return BlockStairsFalling.EnumShape.INNER_RIGHT;
            }
        }

        return BlockStairsFalling.EnumShape.STRAIGHT;
    }
    ...
    {
        return state.getBlock() instanceof BlockStairsFalling;
    }
        ...
        switch (mirrorIn)
        {
            case LEFT_RIGHT:
                ...
                if (enumfacing.getAxis() == EnumFacing.Axis.Z)
                {
                    ...
                    {
                        case OUTER_LEFT:
                            return state.withRotation(Rotation.CLOCKWISE_180).withProperty(SHAPE, BlockStairsFalling.EnumShape.OUTER_RIGHT);
                        case OUTER_RIGHT:
                            return state.withRotation(Rotation.CLOCKWISE_180).withProperty(SHAPE, BlockStairsFalling.EnumShape.OUTER_LEFT);
                        case INNER_RIGHT:
                            return state.withRotation(Rotation.CLOCKWISE_180).withProperty(SHAPE, BlockStairsFalling.EnumShape.INNER_LEFT);
                        case INNER_LEFT:
                            return state.withRotation(Rotation.CLOCKWISE_180).withProperty(SHAPE, BlockStairsFalling.EnumShape.INNER_RIGHT);
                        default:
    ...
    public String getHarvestTool(IBlockState state) {
    	if(state.getMaterial() == Material.WOOD) return "axe";
    	if(state.getMaterial() == Material.GROUND) return "pickaxe";
